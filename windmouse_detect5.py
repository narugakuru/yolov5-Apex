...
def process_img(model, im):
    im = letterbox(im, (area, area), stride=32, auto=True)[0]  # padded resize
...
def get_target(target_list, distance_list):
...
def plan_excute(target_X, target_Y):
    points = []
    # 局内灵敏度步枪8，狙击枪12ww
    # 局内灵敏度5，步长15 步枪效果不错
    wind_mouse(0, 0, target_X, target_Y, G_0=20, W_0=5,
               M_0=30, D_0=50,
    ...
    while True:
        ...
        for i, det in enumerate(pred):
            ...
            if len(det):
                ...
                target_X, target_Y = get_target(target_list, distance_list)
                # 加个距离非零判断 防止抖动
                if is_x2_pressed and (abs(target_X) > 2 or abs(target_Y) > 2):
                    plan_excute(target_X, target_Y)
